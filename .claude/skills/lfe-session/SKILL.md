...
**T9. Permission/persona consistency.** Every persona key in `.agents/permissions.json` (`architect`, `builder`, `inspector`, `archivist`, `scout`) has a corresponding `.docs/protocol/personas/<key>.md` file. `PERSONAS.md` index links to each. No persona file exists without a matching permissions.json key (and vice versa).
